...
from training.network.module.TransformerDecoder import TransformerDecoder
import gc
...
class Network(nn.Module):
    ...
    def forward(self, input, is_runtime=False, style_embedding=None):
        ...
        if self.content_vae_mode == True:
            ...
            bodyContentEmbedding = self.bodyContentEncoder(input_body) if self.use_body_content_encoder else input_body
    ...
    # def forward_with_two_style(self, input, interpolation = False, ratio = None):
    ...
    #     faceStyleEmbedding_0 = self.emotionEncoder(input_emotionEncoder_0)[0]
    #     faceStyleEmbedding_1 = self.emotionEncoder(input_emotionEncoder_1)[0]

    #     if interpolation == False:

    #         if ratio == None:

    #             faceStyleEmbedding_0_repeated = faceStyleEmbedding_0.unsqueeze(1).repeat(1, frame_num // 2, 1)
    #             faceStyleEmbedding_1_repeated = faceStyleEmbedding_1.unsqueeze(1).repeat(1, frame_num - frame_num // 2, 1)

    #             # 두 부분을 연결하여 (batch_size, frame_num, latent_dim) 모양의 faceStyleEmbedding 생성
    #             combined_faceStyleEmbedding = torch.cat([faceStyleEmbedding_0_repeated, faceStyleEmbedding_1_repeated], dim=1)

    #             # bodyContentEmbedding과 결합
    #             hidden_states = torch.cat([combined_faceStyleEmbedding, bodyContentEmbedding], dim=2)

    #             style_transformed_0 = self.style_transform(faceStyleEmbedding_0)
    #             style_transformed_0_expanded = style_transformed_0.unsqueeze(1).repeat(1, frame_num // 2, 1)

    #             style_transformed_1 = self.style_transform(faceStyleEmbedding_1)
    #             style_transformed_1_expanded = style_transformed_1.unsqueeze(1).repeat(1, frame_num - frame_num // 2, 1)

    #             style_transformed_expanded = torch.cat([style_transformed_0_expanded, style_transformed_1_expanded], dim = 1)

    #         else:
    #             weighted_embedding = (1 - ratio) * faceStyleEmbedding_0 + ratio * faceStyleEmbedding_1

    #             # weighted_embedding을 반복하여 faceStyleEmbedding 생성
    #             weighted_embedding_repeated = weighted_embedding.unsqueeze(1).repeat(1, frame_num, 1)

    #             # bodyContentEmbedding과 결합
    #             hidden_states = torch.cat([weighted_embedding_repeated, bodyContentEmbedding], dim=2)

    #             # weighted_embedding에 대해 스타일 변환 및 반복
    #             style_transformed_weighted = self.style_transform(weighted_embedding)
    #             style_transformed_expanded = style_transformed_weighted.unsqueeze(1).repeat(1, frame_num, 1)
    ...
    #         interpolation_frames = frame_num  # frames to interpolate

    #         interpolation_weights = torch.linspace(0, 1, interpolation_frames).unsqueeze(0).unsqueeze(2).to(faceStyleEmbedding_0.device)
    #         interpolated_embeddings = torch.lerp(faceStyleEmbedding_0.unsqueeze(1), faceStyleEmbedding_1.unsqueeze(1), interpolation_weights)

    #         # Concatenate all sections
    #         combined_faceStyleEmbedding = interpolated_embeddings

    #         # Concatenate the face style embeddings with the body content embedding
    #         hidden_states = torch.cat([combined_faceStyleEmbedding, bodyContentEmbedding], dim=2)

    #         style_transformed_0 = self.style_transform(faceStyleEmbedding_0)
    #         style_transformed_1 = self.style_transform(faceStyleEmbedding_1)

    #         interpolation_weights = torch.linspace(0, 1, interpolation_frames).unsqueeze(0).unsqueeze(2).to(style_transformed_0.device)
    #         style_transformed_interpolated = torch.lerp(style_transformed_0.unsqueeze(1), style_transformed_1.unsqueeze(1), interpolation_weights)

    #         # Concatenate all sections
    #         style_transformed_expanded = style_transformed_interpolated


    #     decoder_output = self.decoder(hidden_states, style_transformed_expanded)
    #     blendshape_output = self.bs_map_r(decoder_output)


    #     return_vector =  {
    #         'blendshape_output': blendshape_output,
    #         'faceStyleEmbedding_0': faceStyleEmbedding_0,
    #         'faceStyleEmbedding_1': faceStyleEmbedding_1,
    #     }
    ...
    # def loss_velocity_calculate(self, prediction, original_data, jaw_weight):

    #     expression_velocity_origin = (original_data[:, 1:, 3:] - original_data[:, :-1, 3:])*30
    #     expression_velocity_preidction = (prediction[:, 1:, 3:]- prediction[:, :-1, 3:]) *30

    #     loss_velocity_expression = F.mse_loss(expression_velocity_preidction, expression_velocity_origin)

    #     jaw_velocity_origin = utils.compute_rotational_velocity(original_data[:, :, :3], time_interval=1/30)
    #     jaw_velocity_prediction = utils.compute_rotational_velocity(prediction[:, :, :3], time_interval=1/30)

    #     loss_velocity_jaw = F.mse_loss(jaw_velocity_prediction, jaw_velocity_origin)

    #     loss_velocity = loss_velocity_expression + loss_velocity_jaw * jaw_weight
    ...
    def epoch_process_step4_NoCross(self, epoch, dataloader, dataloader_sub, writer, jaw_weight = 1000, kl_weight = 0.5):
        ...
        for training_data_main, training_data_sub in zip(dataloader, dataloader_sub):
            ...
            # loss_velocity_step1 = self.loss_velocity_calculate(output['blendshape_output'], facial_motion_content_0, jaw_weight)
            ...
            if self.vae_mode == True:
                ...
                # print("std:", torch.exp(0.5 * logvar).mean().item())
                # print("mu std:", mu.std().item())

            if self.content_vae_mode == True:
                quantized_body = output['quantized_body']
                projected_body = output['projected_body']
                quantized_face = output['quantized_face']
                projected_face = output['projected_face']

                body_vq_loss = self.bodyContentEncoder.compute_vq_loss(quantized_body, projected_body)
                face_vq_loss = self.faceContentEncoder.compute_vq_loss(quantized_face, projected_face)

                writer.add_scalar("loss_vq_body/train", body_vq_loss.item(), epoch)
                writer.add_scalar("loss_vq_face/train", face_vq_loss.item(), epoch)
                loss_total += 0.01 * (body_vq_loss + face_vq_loss)
            ...
            if self.content_vae_mode == True:
                ...
                writer.add_scalar("loss_vq_body/train", body_vq_loss.item(), epoch+1)
                writer.add_scalar("loss_vq_face/train", face_vq_loss.item(), epoch+1)
                loss_total += 0.01 * (body_vq_loss + face_vq_loss)
            ...
            writer.add_scalar("Total Loss/train",loss_total, epoch+1)
            self.network_update(loss_total)
            avg_error_train += (loss_total).item() / batch_num
    ...
    def epoch_process_step4_NoAlign(self, epoch, dataloader, dataloader_sub, writer, jaw_weight = 1000, kl_weight = 0.5):
        avg_error_train = 0.
        batch_num = len(dataloader)
        batch_size = dataloader.batch_size
        ...
        for training_data_main, training_data_sub in zip(dataloader, dataloader_sub):
            ...
            loss_total = loss_reconstruction_step1 * 5.0 + loss_content_step1 * 0.5  + loss_style_step1 * 1.0
            ...
            loss_total = loss_reconstruction_step3 * 5.0 + loss_content_step3 * 0.5  + loss_style_step3 *1.0
            ...
            loss_total = loss_total+loss_content_step4 * 0.1 + loss_style_step4 *0.1
    ...
    def epoch_process_step4_origin(self, epoch, dataloader, dataloader_sub, writer, jaw_weight = 1000, kl_weight = 0.5):
        ...
        for training_data_main, training_data_sub in zip(dataloader, dataloader_sub):
            ...
            loss_total = loss_reconstruction_step1 * 5.0 + loss_align_step1 * 0.5
            ...
            loss_total = loss_reconstruction_step3 * 5.0 + loss_align_step3 * 0.5
    ...
    def train_network(self, dataloader, epoch_num, writer, save_dir, save_step, dataloader_sub = None, mode = "origin"):
        ...
        if mode == "origin":
            ...
            for epoch in range(1, epoch_num+1, 2):
                self.epoch_process_step4_NoAlign(epoch, dataloader, dataloader_sub, writer)
            ...
            for epoch in range(1, epoch_num+1, 2):
                self.epoch_process_step4_NoCross(epoch, dataloader, dataloader_sub, writer)
        ...
        # elif mode == "NoConCon":
        #     for epoch in range(1, epoch_num+1, 2):
        #         self.epoch_process_step4_NoConCon(epoch, dataloader, dataloader_sub, writer)

        #         if (epoch > 0 and (epoch % save_step == save_step - 1 or epoch % save_step == save_step-2)) or epoch == 1:
        #             save_path = save_dir + "/model"+"_"+str(epoch)+"epoch.pth"
        #             torch.save(self, save_path)

        # elif mode == "NoStyleCon":
        #     for epoch in range(1, epoch_num+1, 2):
        #         self.epoch_process_step4_NoStyleCon(epoch, dataloader, dataloader_sub, writer)

        #         if (epoch > 0 and (epoch % save_step == save_step - 1 or epoch % save_step == save_step-2)) or epoch == 1:
        #             save_path = save_dir + "/model"+"_"+str(epoch)+"epoch.pth"
        #             torch.save(self, save_path)
        elif mode == "NoConsistnecy":
            for epoch in range(1, epoch_num+1, 1):
                self.epoch_process_step4_NoConsistnecy(epoch, dataloader, dataloader_sub, writer)
